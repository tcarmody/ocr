...
public struct ClaudePostProcessor: Sendable {
    ...
    /// budget exhausted, network failure). Returns a `Result` with
    /// `accepted: false` when the call ran but the guardrail rejected
    /// the candidate — caller may want to log that distinct case for
    /// the editor's correction trail; either way, the `corrected`
    /// field carries the right text to use.
    public func correct(
        text: String,
        languages: [BCP47]
    ) async -> Result? {
