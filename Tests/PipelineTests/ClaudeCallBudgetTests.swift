...
/// `ClaudeCallBudget` is the shared per-book cost cap actor every
...
final class ClaudeCallBudgetTests: XCTestCase {

    func test_grants_up_to_the_cap_then_refuses() async {
        let budget = ClaudeCallBudget(cap: 3)
        for _ in 0..<3 {
    ...
    func test_isExhausted_flips_at_the_cap() async {
        let budget = ClaudeCallBudget(cap: 1)
    ...
    func test_zero_cap_refuses_immediately() async {
        let budget = ClaudeCallBudget(cap: 0)
        let ok = await budget.tryConsume()
        XCTAssertFalse(ok)
    }
